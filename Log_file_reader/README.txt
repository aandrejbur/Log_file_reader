...
После считывания первого блока данных, передает считанную строку в очередь поиска(очереди организованы на основе двухстороннего списка) и увеличивает семафор для потока поиска. В случае если в очереди поиска более 10К строк, поток чтения останавливается на 1000 мкС, Это необходимо для контроля расхода оперативной памяти, тогда когда поиск по долгу не может найти строки определенного вхождения.
Поток считывает из блока символы в строку, максимальный размер строки 32 кило байта. Если вдруг строка большего размера то дополнительно выделяется еще по 32 килобайта всякий раз когда буфер для строки переполнен, в случае когда буфер достигает размера 1 МБ, строка режется, и в дальнейшем все последующие строки размером больше 1Мб будт делиться на этой границе. Размер строки можно указать и больший, но в таком случае Приложение сожрет слишком много оперативной памяти, для лог файлов в одну строку потребуется переписаывать пиложение для определения разделителей строк. В своей практике еще не разу не встречал лог файлы размером более 2 гб написанные в ону строку, это как минимум нечитаемо. и разница между символом пробела и новой строки в ASCII кодировке, не дает никакого преимущества.
...
1-ый режим - режим генерации файла. Запуск программы без параметров командной строки приводит к генерации тестового файла размером 2ГБ.
...
1 - обработку вызова справки (не было выхода из программы и происходила ошибка)
